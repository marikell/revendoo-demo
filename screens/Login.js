...
import { View, TouchableOpacity, TextInput, AsyncStorage, Alert, Text } from 'react-native';
import { API_URL } from '../services/config';
import axios from 'axios';
import styles from '../styles';

const handleSubmit = (values, navigation) => {
    axios.post(`${API_URL}/auth/signin`, values).then(res => {
        if (res.status == 200) {

            Alert.alert('', 'Login realizado com sucesso.')

            AsyncStorage.setItem('token', res.data).then(value => {
